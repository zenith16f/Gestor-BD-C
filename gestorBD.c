...
void anadirRegistro(BaseDatos *base, char *nombreTabla)
{
	...
	while (numNuevosRegistros-- > 0)
	{
		...
		for (int i = 0; i < tabla->numCampos; i++)
		{
			...
			{
				// Logica similar al arbol binario para obtener la cadena
				getchar(); // Limpia el salto de línea pendiente en el buffer
				fgets((char *)nuevoRegistro->datos[i], tabla->campos[i].longitud, stdin);

				// Elimina el salto de línea final si quedó
				char *pos;
				if ((pos = strchr((char *)nuevoRegistro->datos[i], '\n')) != NULL)
					*pos = '\0';
			}
		}
...
void guardarRegistroEnArchivo(Tabla *tabla, Registro *registro)
{
	...
	sprintf(nombreArchivo, "%s.txt", tabla->nombre); // El archivo se llama igual que la tabla

	FILE *archivo = fopen(nombreArchivo, "a"); // Abrir en modo append
	if (!archivo)
	{
	...
	// Escribir cada campo separado por coma
	for (int i = 0; i < tabla->numCampos; i++)
	{
	...
	fprintf(archivo, "\n"); // Nueva línea al final de cada registro
...
void cargarEstructuraDesdeArchivo(Tabla *tabla, const char *rutaSchema){
	...
	for (int i = 0; i < numCampos; i++)
	{
		...
		tabla->campos[i].nombre = (char *)malloc(50);
		strcpy(tabla->campos[i].nombre, token);

		token = strtok(NULL, ",");
		if (!token)
			continue;
		if (strcmp(token, "ENTERO") == 0)
			tabla->campos[i].tipo = ENTERO;
		else
			tabla->campos[i].tipo = VARCHAR;

		token = strtok(NULL, ",");
		tabla->campos[i].longitud = token ? atoi(token) : 100;

		tabla->campos[i].siguiente = (i < numCampos - 1) ? &tabla->campos[i + 1] : NULL;
	}
